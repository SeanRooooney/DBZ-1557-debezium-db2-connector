...
This connector was added in Debezium release 1.1.x.
...
The functionality of the connector is based upon the ASN Capture/Apply agents 
that enable SQL Replication within DB2:
https://www.ibm.com/support/pages/q-replication-and-sql-replication-product-documentation-pdf-format-version-101-linux-unix-and-windows
...
The connector has been tested with Db2/Linux 11.5.0.0, but our expectation is that the model would work for Windows, AIX and zOS as wll.
...
db2 control commands would have the same effect.
...
The database needs to have been recently backed-up so doing following if this is not the case:
...
We assume that the db2 is installed on the db2inst1 user
...
|`DECFLOAT`
|`JDBC type '1111 Currently NOT Supported
|n/a
| WARN io.debezium.relational.TableSchemaBuilder  - Unexpected JDBC type '1111' for column '   ' that will be ignored
...
If you've already installed https://zookeeper.apache.org[Zookeeper], http://kafka.apache.org/[Kafka], and http://kafka.apache.org/documentation.html#connect[Kafka Connect], then using Debezium's Db2` connector is easy.
...
If immutable containers are your thing, then check out https://hub.docker.com/r/debezium/[Debezium's Docker images] for Zookeeper, Kafka and Kafka Connect with the Db2 connector already pre-installed and ready to go.
...
Using the Db2 connector is straightforward. Here is an example of the configuration for a connector instance that monitors a Db2 server at port 3306 on 192.168.99.100, which we logically name `fullfillment`:
...
|`connector.class`
|
|The name of the Java class for the connector. Always use a value of `io.debezium.connector.sqlserver.SqlServerConnector` for the Db2 connector.
...
This is used to define the timezone of the transaction timestamp (ts_ms) retrieved from the server (which is actually not zoned). Default value is unset. +
When unset, default behavior is to use the timezone of the VM running the Debezium connector.  +
...
Be sure to consult the http://kafka.apache.org/documentation.html[Kafka documentation] for all of the configuration properties for Kafka producers and consumers. (The Db2 connector does use the http://kafka.apache.org/documentation.html#newconsumerconfigs[new consumer].)
